...
signal(out(G), 0) :- type(G, andGate), signal(in(1,G), 0); type(G, andGate), signal(in(1,G), 1), signal(in(2,G), 0).


signal(out(G), 1) :- type(G, orGate), signal(in(1,G), 1); type(G, orGate), signal(in(1,G), 0), signal(in(2,G), 1).
